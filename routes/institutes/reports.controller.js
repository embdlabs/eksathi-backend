...
const createReport = async (req, res) => {
    ...
    const { email, item_id, item_type, reason, comment } = req.body;

    if (!email || !item_id || !item_type || !reason || !comment) {
        return res.status(409).json({ message: 'Invalid email or Item ID or Item Type or comment' });
    }

    try {
        const user_id = await getUserIDByEmail(db, email);

        const sql = `
        INSERT INTO ${db}.reports (user_id, item_id, item_type, reason, comment)
        VALUES (?, ?, ?, ?, ?)
        `;
...
const getReports = async (req, res) => {
    ...
    try {
        const user_id = await getUserIDByEmail(db, email).catch(err => {
            console.error('Error getting reports:', err.stack);
            return res.status(403).json({ message: "Invalid User" });
        });
        const sql = `
        SELECT * FROM ${db}.reports WHERE user_id =?
        `;
        mysqlcon.query(sql, [user_id], (err, result) => {
            if (err) {
                console.error('Error getting reports:', err.stack);
                return res.status(500).json({ message: 'Internal Server Error' });
            }
...
const deleteContent = (req, res) => {
    ...
    if (req.user.role !== 'admin') {
      ...
      connection.query(sql, [contentId], (err, results) => {
        if (err) {
      ...
      connection.query(sql, [contentType, contentId], (err, results) => {
        if (err) {
          console.error('Error executing SQL query:', err.stack);
          return res.status(500).json({ error: 'Internal Server Error' });
        }
