...
const createDatabaseName = (email) => {
  const databaseName = `eks_${email.split("@")[0]}_${Math.floor(Math.random() * 100000)}`;
  return databaseName;
}
        ...
        mysqlcon.query(sql, (error, results) => {
          if (error) {
            ...
            if (results.length) {
              ...
              console.log("Error: Invalid credentials, Path not found")
              reject("Error: Invalid credentials");
            }
          }
        });
